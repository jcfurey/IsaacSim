...
public:
    static bool compute(OgnROS2PublishLaserScanDatabase& db)
    {
        ...
        if (!state.m_publisher)
        {
            ...
            if (qosProfile.empty())
            {
                // ROS 2 convention for sensor_msgs/LaserScan: use "sensor data" QoS
                // (best-effort) so a slow/absent subscriber cannot back up the DDS queue
                // and add latency to the live scan stream. Matches the Image/CameraInfo
                // publishers. Only applies when no explicit qosProfile was supplied.
